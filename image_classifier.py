...
def create_dataset(dataset_size=MODEL_SIZE):
    ...
    if not os.path.exists('dataset.p'):
        ...
        for car in car_dirs[0:dataset_size]:
            ...
            # flip
            car_feature_map.append(extract_features(cv2.flip(cv2.cvtColor(cv2.imread(car), cv2.COLOR_BGR2RGB),
                                                             flipCode=1), color_space=COLOR_SPACE))
            # blur
            car_feature_map.append(extract_features(cv2.blur(cv2.cvtColor(cv2.imread(car), cv2.COLOR_BGR2RGB),
                                                             (5, 5)), color_space=COLOR_SPACE))
            # histogram_equalization
            car_yuv = cv2.cvtColor(cv2.imread(car), cv2.COLOR_BGR2YUV)
            car_yuv[:, :, 0] = cv2.equalizeHist(car_yuv[:, :, 0])
            car_feature_map.append(extract_features(cv2.cvtColor(car_yuv, cv2.COLOR_YUV2RGB), color_space=COLOR_SPACE))
            # original image
            car_feature_map.append(extract_features(cv2.cvtColor(cv2.imread(car), cv2.COLOR_BGR2RGB),
        ...
        for non_car in non_car_dirs[0:dataset_size]:
            ...
            # operations: blur, histogram equalization, flip
            # flip
            non_car_feature_map.append(extract_features(cv2.flip(cv2.cvtColor(cv2.imread(non_car), cv2.COLOR_BGR2RGB),
                                                                 flipCode=1), color_space=COLOR_SPACE))
            # blur
            non_car_feature_map.append(extract_features(cv2.blur(cv2.cvtColor(cv2.imread(non_car), cv2.COLOR_BGR2RGB),
                                                                 (5, 5)), color_space=COLOR_SPACE))
            # histogram_equalization
            non_car_yuv = cv2.cvtColor(cv2.imread(non_car), cv2.COLOR_BGR2YUV)
            non_car_yuv[:, :, 0] = cv2.equalizeHist(non_car_yuv[:, :, 0])
            non_car_feature_map.append(extract_features(cv2.cvtColor(non_car_yuv, cv2.COLOR_YUV2RGB),
                                                        color_space=COLOR_SPACE))
            # original image
            non_car_feature_map.append(extract_features(cv2.cvtColor(cv2.imread(non_car), cv2.COLOR_BGR2RGB),
                                                        color_space=COLOR_SPACE))
...
def fit_svm(dataset):
    ...
    with open('svm_clf_scaler_default.p', mode='wb') as f:
...
def get_classifier():
    if os.path.exists('svm_clf_scaler_default.p'):
        with open('svm_clf_scaler_default.p', mode='rb') as f:
